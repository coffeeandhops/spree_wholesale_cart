...
RSpec.describe Spree::WholesalePrice, type: :model do
  describe '#amount=' do
    let(:price) { Spree::WholesalePrice.new }
    let(:amount) { '3,0A0' }
  ...
  describe 'validations' do
    subject { Spree::WholesalePrice.new variant: variant, amount: amount }
    ...
    context 'when the amount is greater than maximum amount' do
      let(:amount) { Spree::WholesalePrice::MAXIMUM_AMOUNT + 1 }

      it 'populates errors' do
        subject.valid?
        expect(subject.errors.messages[:amount].first).to eq "must be less than or equal to #{Spree::WholesalePrice::MAXIMUM_AMOUNT}"
      end
    end

    context 'when the amount is between 0 and the maximum amount' do
      let(:amount) { Spree::WholesalePrice::MAXIMUM_AMOUNT }

      it { is_expected.to be_valid }
    end
  end

  describe '#wholesale_price_including_vat_for(zone)' do
    subject(:price_with_vat) { price.wholesale_price_including_vat_for(price_options) }
    ...
    let(:price) { Spree::WholesalePrice.new variant: variant, amount: amount }
  ...
  describe '#display_wholesale_price_including_vat_for(zone)' do
    subject { Spree::WholesalePrice.new amount: 10 }

    it 'calls #price_including_vat_for' do
      expect(subject).to receive(:wholesale_price_including_vat_for)
      subject.display_wholesale_price_including_vat_for(nil)
    end
  end
end
